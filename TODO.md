...
## Why These Tasks
These are the same three tasks from 2026-06-16 that went unstarted; closing them out is the only remaining gate before the full migration can be marked complete. Tasks 1 and 2 cover the class-level test gap in `transcript.sanitizer.spec.ts`, and Task 3 is a one-line module change that ensures the service is injectable by future consumers.
