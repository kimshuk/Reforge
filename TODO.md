...
- [ ] **Close Step 5: implement the `sanitize()` method body in TranscriptSanitizer**
  - All the heavy lifting is done — `sanitizeSnippetList()` and `buildSegments()` are fully implemented. This task is a single method body that calls them in sequence: run snippets through `sanitizeSnippetList()`, capture the count, pass the result to `buildSegments()`, and return the three-field `SanitizedTranscript` object.
...
## Why These Tasks
Task 1 is a single closing move whose return shape is the typed contract between the sanitizer and all downstream callers — it must be green before LlmService can safely import from it. Tasks 2 and 3 front-load the two hardest surfaces of Step 6 (the prompt/schema/types contract and the HTTP + error-mapping logic) so that AnalyzeService in Step 7 can wire them without revisiting LlmService internals.
